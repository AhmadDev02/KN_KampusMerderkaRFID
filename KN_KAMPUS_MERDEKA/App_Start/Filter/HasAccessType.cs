            ...
            try
            {
                ...
                if (context.Session != null)
                {
                    if (CurrentSession.getPrincipal != null)
                    {
                        ...
                        for (int i = 0; i < CurrentSession.getPrincipal.roles.Count; i++)
                        {
                            mRoleAccess RoleAccessDat = mRoleAccessCustomBL.GetPrivilegeUserUrlWithoutMenus(CurrentSession.getPrincipal.roles[i], txtUrl);
