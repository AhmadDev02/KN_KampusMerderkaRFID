...
(function ($) {
    $.extend({
        ...
        confirmMessage: function (title, message, confirmtext, callbackaction, callbackcancel = () =>{}) {
            Swal.fire({
                title: title,
                text: message,
                type: 'warning',
        ...
        parseToJSONDateFromDate: function (txtValue, txtFormat) {
            try {
                return '/Date(' + new Date(txtValue).getTime() + ')/';
            } catch (ex) {
                return "-";
            }
        },
...
$("#darkSwitch").prop('checked', darkMode == 1);
if (darkMode == 1) {
